...
"""
# 設定
app = LambdaAPI(token_password, session_password)
def lambda_handler(event, _):
    return app.request(event)

@app.post("/test", session=False, token=False)
def test()
...
class LambdaAPI:
    def __init__(self, token_password, session_password):
        self.funcs = {}
        self.id = ""
    ...
    def __api(self, url, method, token, session):
        def _wrapper(func):
            if url not in self.funcs:
                self.funcs[url] = {}
            self.funcs[url][method] = {"func": func, "token": token, "session": session}
